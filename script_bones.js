...
const precoVarejo = 39.9; // Preço padrão de varejo
const precoAtacado = 18.0; // Preço padrão de atacado
const precoCamiseta = 49.9; // Preço de varejo para camisetas
const aparecerPrecoVarejo = true; // Mostrar ou esconder preço de varejo
const aparecerPrecoAtacado = false; // Mostrar ou esconder preço de atacado

const numeroCelular = "5511940180611"; // Número de WhatsApp
const mensagemWhatsApp = "Olá, vim do seu catálogo."; // Mensagem de WhatsApp
const mostrarNumeroCelular = true; // Mostrar ou esconder número de celular

const nomeEmpresa = "D.J Outlet"; // Nome da empresa
...
const banner = true; // Mostrar ou ocultar o banner
...
function displayProducts(products) {
  ...
  products.forEach((product) => {
    ...
    const productCategory = product.Category; // Pegar a categoria do produto
    ...
    // Exibir o preço de acordo com a categoria do produto
    if (productCategory === categories.CAMISETAS) {
      const priceCamisetaPara = document.createElement("p");
      priceCamisetaPara.textContent = `Camiseta: R$${precoCamiseta
        .toFixed(2)
        .replace(".", ",")}`;
      console.log("Exibindo preço de camiseta:", priceCamisetaPara.textContent);
      priceContainer.appendChild(priceCamisetaPara);
    } else {
      // Mostrar preço de varejo se não for camiseta
      if (aparecerPrecoVarejo) {
        const priceVarejoPara = document.createElement("p");
        priceVarejoPara.textContent = `Varejo: R$${precoVarejo
          .toFixed(2)
          .replace(".", ",")}`;
        console.log("Exibindo preço de varejo:", priceVarejoPara.textContent);
        priceContainer.appendChild(priceVarejoPara);
      }
    }

    // Mostrar preço de atacado
